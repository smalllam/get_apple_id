...
/**
 * 从idusr.com API直接获取Apple ID账号信息
 * 使用Node.js原生https模块
 */
async function extractAppleIDs(url: string = 'https://idusr.com/data/appleid.txt'): Promise<AppleID[]> {
      ...
      res.on('end', () => {
        try {
          ...
          jsonData.forEach((item: any) => {
            // 检查是否是有效账号
            if (!item.email || !item.password || item.email === 'N/A') {
              return;
            }
