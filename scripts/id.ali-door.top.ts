...
async function extractAppleIDs(url: string = 'https://id.ali-door.top/share/umoKGeJwOa'): Promise<AppleID[]> {
  // 启动浏览器
  ...
  const tempHtmlPath = path.join(process.cwd(), 'id.ali-door.top.html');
    ...
    const accounts = await localPage.evaluate(() => {
      ...
      try {
        ...
        cardBodies.forEach((cardBody) => {
          // 查找所有带有"正常"文本的标签
          const statusElements = cardBody.querySelectorAll('.badge.bg-green');
          let isNormal = false;
          ...
          if (isNormal) {
             // 提取地区
             const region = cardBody.querySelector('.badge.bg-indigo');
             let regionText = '';
             if (region) {
            ...
            if (buttons.length >= 2) {
              ...
              if (account && password) {
                result.push({ account, password, region: regionText });
              }
            }
